...
### Intervention 4: Education (mediation version) ###

# mediation : intervene on partnership
# so that we can get the direct effect

# keep a group of women
...
for(bs in 1:bssize) {
  ...
  for(t in 1987:2008) {
    ...
    ## !! predict partnership !!Blocked due to mediation control!!!!####
    fill.sample.2$partner.single[fill.sample.2$year==t] <- fill.sample$partner.single[fill.sample$year==t] 
    fill.sample.2$partner.cohab[fill.sample.2$year==t] <- fill.sample$partner.cohab[fill.sample$year==t]
    fill.sample.2$partner.married[fill.sample.2$year==t] <- fill.sample$partner.married[fill.sample$year==t]
...
## Mediation analysis ##
output.int4 <- read.csv(file='int4_output.csv')

# Total Effect
TE <- output.int4$babies.nc - output.int4$babies.int
plot(TE,type='l')

# Direct Effect
CDE <- output.int4$babies.nc - babies.int
plot(CDE,type='l')
...
write.csv(cbind(CDE/TE, PE/TE),file='int4_mediation_1.csv')
