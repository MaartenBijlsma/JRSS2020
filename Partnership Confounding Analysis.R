...
### Intervention 3: Partnership (confounding check) ###

# a decrease in the proportion of women who are married 
# (and corresponding increase in the proportions single and cohabiting, 
# approximately equal to a 50% reduction in the stock of married women over the study period)

# In this version of the analysis
# we introduce a hypothetical confounder that changes the
# estimate, so that we can see how robust our estimates are
...
# we create a confounder score
# based on totalbirth and totalmarried

# so we need to make totalmarried first
fill.dat$totalmarried <- NA
fill.dat$totalcohab <- NA
fill.dat$totalsingle <- NA
for(id in fill.dat$bcsid) {
  
  fill.dat$totalmarried[fill.dat$bcsid==id] <- cumsum(fill.dat$partner.married[fill.dat$bcsid==id])
  fill.dat$totalcohab[fill.dat$bcsid==id] <- cumsum(fill.dat$partner.cohab[fill.dat$bcsid==id])
  fill.dat$totalsingle[fill.dat$bcsid==id] <- cumsum(fill.dat$partner.single[fill.dat$bcsid==id])
  
  print(id)
  
}

# create a confounding score
# based on marriage and births
# later, we only use the value of respondents
# at the end of follow-up.
# and variance is added in the g-formula loop.
fill.dat$confounder.score.base <- 0 + 3*fill.dat$totalbirth + 1*fill.dat$totalmarried
...
# piece of code needed for confounder bit
fill.dat.1986 <- fill.dat[fill.dat$year==1986,]
# /end
...
for(bs in 1:bssize) {
  ...
  # add randomness to confounder score
  # randomness determines strength of relation
  fill.dat$confounder.score.base.2 <- fill.dat$confounder.score.base  + rnorm(length(fill.dat$bcsid),0,1)
  # strong version has SD of 1, medium version SD of 3.5
  # light is SD of 7
  
  # assign confounder score of 2008 to all years
  for(id in unique(fill.dat$bcsid[fill.dat$year==2008])) {
    
    fill.dat$confounder.score[fill.dat$bcsid==id] <- fill.dat$confounder.score.base.2[fill.dat$bcsid==id & fill.dat$year==2008]
    # we use .base because otherwise the variance expands
    # every time we add an rnorm on top of an rnorm
  }
  
  # start normal G-formula code
...
# these are estimates found in the analysis without the confounder
# I place them here for easy comparison
nonconfounded <- c(NA, 1.0116944, 0.9995574, 0.9888659, 0.9764879, 0.9668641, 0.9541816,
                   0.9413965, 0.9294037, 0.9204377, 0.9093216, 0.9018676, 0.8956564, 0.8888710,
                   0.8830191, 0.8817070, 0.8792601, 0.8761380, 0.8731563, 0.8700899, 0.8670979,
                   0.8642544, 0.8619120)

plot(16:38,nonconfounded,type='l',lty=1,lwd=3, col='grey',xlab='Age',ylab='Ratio of number of children ever born',
     main='Marriage Scenario (strong confounder)',ylim=c(0.70,1.30),bty='l')
lines(16:38,c(NA,tfr.ncint[-1]),type='l',lty=1,lwd=2)
...
legend(legend=c('Original estimate','Estimate with confounding', '95% confidence bounds'),20,1.3,lwd=c(3,2,1),
       lty=c(1,1,2),bty='n',col=c('grey','black','black'))
...
# note that numbers are different, so should check % distributions
apply(babytable[,,23],2,mean)/sum(apply(babytable[,,23],2,mean))
table(fill.dat$totalbirth[fill.dat$year==2008])/sum(table(fill.dat$totalbirth[fill.dat$year==2008]))
