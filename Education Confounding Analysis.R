...
### Intervention: Education (confounding check) ###
...
# based on totalbirth, totaledu, totaljobless and totaljobfull

fill.dat$totaljobless <- NA
fill.dat$totaljobpart <- NA
fill.dat$totaljobfull <- NA
for(id in fill.dat$bcsid) {
  
  fill.dat$totaljobless[fill.dat$bcsid==id] <- cumsum(fill.dat$job.less[fill.dat$bcsid==id])
  fill.dat$totaljobpart[fill.dat$bcsid==id] <- cumsum(fill.dat$job.part[fill.dat$bcsid==id])
  fill.dat$totaljobfull[fill.dat$bcsid==id] <- cumsum(fill.dat$job.full[fill.dat$bcsid==id])
  
  print(id)
  
}

# in the g-formula loop we add randomness to the score
# to calibrate the confounding score strength
fill.dat$confounder.score.base <- 0 + 3*fill.dat$totalbirth + -1.96*fill.dat$totaledu +
  0*fill.dat$totaljobless + -1.13*fill.dat$totaljobfull + -0.64*fill.dat$totaljobpart
...
for(bs in 1:bssize) {
  ...
  fill.dat$confounder.score.base.2 <- fill.dat$confounder.score.base  + rnorm(length(fill.dat$bcsid),0,5)
  # strong version has SD of 0.1, medium version rnorm SD of 5
  # light is rnorm SD of 10
  # at rnorm of 10, the effect dissapears at the end of follow-up
  # so they've caught up then
...
# predicted values in the analysis
# without the artificial confounder
nonconfounded <- c(NA, 1.0115991, 0.9357619, 0.8686884, 0.8228542, 0.7890726, 0.7641958,
                   0.7463063, 0.7472197, 0.7548572, 0.7690794, 0.7856564, 0.8046661, 0.8228491,
                   0.8410403, 0.8536141, 0.8661344, 0.8777808, 0.8888100, 0.8987425, 0.9084051,
                   0.9171578, 0.9257166)

plot(16:38,nonconfounded,type='l',lty=1,lwd=3, col='grey',xlab='Age',ylab='Ratio of number of children ever born',
     main='Education Scenario (strong confounder)',ylim=c(0.70,1.30),bty='l')
