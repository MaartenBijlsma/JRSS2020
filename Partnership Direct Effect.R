...
### Intervention: Partnership (mediation version) ###

# To perform mediation analysis, we take the values
# that the same person had in the natural course
...
## Start for-loop here: I intend to bootstrap 999 times ##
bssize <- 999
...
household.dat <- matrix(rep(NA,bssize*6),ncol=6)
...
for(bs in 1:bssize) {
  ...
  for(t in 1987:2008) {
    ...
    # to get the direct effect, I draw employment
    # from the natural course (so it is unaffected by
    # the intervention)
    # since the data should be ordered in the same way
    # with the same individuals in the same spot
    # this should work out quite simply as follows:
    fill.sample.2$job.full[fill.sample.2$year==t] <- fill.sample$job.full[fill.sample$year==t]
    fill.sample.2$job.part[fill.sample.2$year==t] <- fill.sample$job.part[fill.sample$year==t]
    fill.sample.2$job.less[fill.sample.2$year==t] <- fill.sample$job.less[fill.sample$year==t]
    fill.sample.2$job.edu[fill.sample.2$year==t] <- fill.sample$job.edu[fill.sample$year==t]
    fill.sample.2$job.perdis[fill.sample.2$year==t] <- fill.sample$job.perdis[fill.sample$year==t]
    ...
    ## just like with employment determine job ses by drawing it from the natural course
    fill.sample.2$jobind[fill.sample.2$year==t] <- ifelse(fill.sample.2$job.full[fill.sample.2$year==t]==1 |
                                                          ...
                                                          1,0)
    fill.sample.2$jobses.high[fill.sample.2$year==t & fill.sample.2$jobind==1] <- fill.sample$jobses.high[fill.sample$year==t & fill.sample$jobind==1]
    fill.sample.2$jobses.low[fill.sample.2$year==t & fill.sample.2$jobind==1] <- fill.sample$jobses.low[fill.sample$year==t & fill.sample$jobind==1]
    ...
    #######
    ## INTERVENTION ##
    # take 15% of married women at random
    marn <- length(fill.sample.2$idnr[fill.sample.2$year==t & fill.sample.2$partner.married==1])
    fill.sample.2$marselect[fill.sample.2$year==t & fill.sample.2$partner.married==1] <- rbinom(marn,1,prob=0.15)
    
    # since marselect is only given to a select group of people
    # set it to 0 also for those who have NA
    fill.sample.2$marselect[is.na(fill.sample.2$marselect)] <- 0
    
    # make these women unmarried
    fill.sample.2$partner.married[fill.sample.2$year==t & fill.sample.2$partner.married==1 &
                                    fill.sample.2$marselect==1] <- 0
    
    # then make half of these women cohabit and half of them single
    # I do this by making half cohabit. The remainder gets single
    marselectn <- length(fill.sample.2$partner.cohab[fill.sample.2$year==t &
                                                       fill.sample.2$marselect==1])
    fill.sample.2$partner.cohab[fill.sample.2$year==t &
                                  fill.sample.2$marselect==1][1:floor(marselectn/2)] <- 1
    fill.sample.2$partner.single[fill.sample.2$year==t & fill.sample.2$marselect==1 &
                                   fill.sample.2$partner.cohab==0] <- 1
    fill.sample.2$marselect <- NULL
    marselect <- marn <- NULL
    #######
  ...
  # how many people are single, cohabiting and married?
  # in each scenario?
  household.dat[bs,1] <- mean(fill.sample$partner.single,na.rm=T)
  household.dat[bs,2] <- mean(fill.sample.2$partner.single,na.rm=T)
  household.dat[bs,3] <- mean(fill.sample$partner.cohab,na.rm=T)
  household.dat[bs,4] <- mean(fill.sample.2$partner.cohab,na.rm=T)
  household.dat[bs,5] <- mean(fill.sample$partner.married,na.rm=T)
  household.dat[bs,6] <- mean(fill.sample.2$partner.married,na.rm=T)
...
plot(babies.nc,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership')
lines(babies.int,lwd=2,col='red')
legend(legend=c('natural course', 'intervention'),1,5000,lwd=2,
       col=c('black','red'))

plot(tfr.nc,type='l',lwd=2)
lines(tfr.int,lwd=2,col='red')
...
# import output of total effect
output.int3.nc.TE <- read.csv(file='int3_output.nc.csv')[-1]
output.int3.int1.TE <- read.csv(file='int3_output.int1.csv')[-1]

int.1.TE <- apply(output.int3.int1.TE - output.int3.nc.TE,2,mean)

plot(int.1.TE,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership (TE)',col='red',ylim=c(-800,200))

int.1.TE.CIlo <- apply(output.int3.int1.TE - output.int3.nc.TE,2,quantile,probs=0.025)
int.1.TE.CIup <- apply(output.int3.int1.TE - output.int3.nc.TE,2,quantile,probs=0.975)

lines(int.1.TE.CIlo,lwd=2,col='red',lty=2)
lines(int.1.TE.CIup,lwd=2,col='red',lty=2)

## Direct Effect ##

int.1.CDE <- apply(output.dat[,2,] - output.dat[,1,],2,mean)

plot(int.1.CDE,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership (CDE)',col='red',ylim=c(-800,200))

int.1.CDE.CIlo <- apply(output.dat[,2,] - output.dat[,1,],2,quantile,probs=0.025)
int.1.CDE.CIup <- apply(output.dat[,2,] - output.dat[,1,],2,quantile,probs=0.975)

lines(int.1.CDE.CIlo,lwd=2,col='red',lty=2)
lines(int.1.CDE.CIup,lwd=2,col='red',lty=2)

## Indirect Effect (PE)

int.1.PE <- int.1.TE - int.1.CDE

plot(int.1.PE,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership (PE)',col='red',ylim=c(-550,400))

# in percentages of each total effect (CDE / TE & PE / TE)

plot(int.1.CDE / int.1.TE,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership (CDE %)',col='red',ylim=c(0,4))

plot(int.1.PE / int.1.TE,type='l',lwd=2,xlab='years since age 15',ylab='number of babies',
     main='Intervention 3: Partnership (PE %)',col='red',ylim=c(0,4))


# save output
med.dat <- data.frame(cbind(round((int.1.CDE / int.1.TE)*100),
                            round((int.1.PE / int.1.TE)*100)))
names(med.dat) <- c('CDE','PE')

write.csv(med.dat,file='Partnership Mediation.csv')

### parities

babytable.int3.nc.TE <- read.csv(file='int3_babytable.nc.csv')[-1]
babytable.int3.int1.TE <- read.csv(file='int3_babytable.int1.csv')[-1]

# total effect
int1.TE <- round(apply(babytable.int3.int1.TE  - babytable.int3.nc.TE,2,mean))

# direct effect
int1.CDE <- round(apply(babytable.2[,,23]  - babytable[,,23],2,mean))

# indirect effect
int1.PE <- round(int1.TE - int1.CDE)

# direct effect % of TE 
round(int1.CDE / int1.TE, digits=2)

# indirect effect % of TE
round(int1.PE / int1.TE, digits=2)
