...
plot(16:38,c(NA,tfr.ncint[-1]),type='l',lty=1,lwd=2,xlab='Age',ylab='Ratio of number of children ever born',
     main='Marriage Scenario',ylim=c(0.70,1.30),bty='l')
...
lines(c(NA,apply(output.dat[,4,]/output.dat[,3,],2,applyquantile95ci)[1,-1]),lty=2)
lines(c(NA,apply(output.dat[,4,]/output.dat[,3,],2,applyquantile95ci)[2,-1]),lty=2)
...
write.csv(output.dat[,1,],file='int3_output.nc.csv')
write.csv(output.dat[,2,],file='int3_output.int1.csv')

write.csv(babytable[,,23],file='int3_babytable.nc.csv')
write.csv(babytable.2[,,23],file='int3_babytable.int1.csv')
