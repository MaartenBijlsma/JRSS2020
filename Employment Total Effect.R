...
# an increase in the proportion of women who are employed
# immediately after birth (assuming that all women work full time one year 
# after their birth, for at least one year).
...
for(bs in 1:bssize) {
  ...
  for(t in 1987:2008) {
    ...
    ### intervention for women who had a birth in this year ###
    ## and who were in fulltime, partime or jobless category
    # not in school or permanently disabled #
    fill.sample.2$birthselect[fill.sample.2$year==t &
                                fill.sample.2$birth==1
                              & (fill.sample.2$job.full==1 |
                                   fill.sample.2$job.part==1 |
                                   fill.sample.2$job.less==1)] <- 1    
    fill.sample.2$job.full[fill.sample.2$year==t & fill.sample.2$birthselect==1] <- 1
    fill.sample.2$job.part[fill.sample.2$year==t & fill.sample.2$birthselect==1] <- 0
    fill.sample.2$job.less[fill.sample.2$year==t & fill.sample.2$birthselect==1] <- 0
    fill.sample.2$job.edu[fill.sample.2$year==t & fill.sample.2$birthselect==1] <- 0
    fill.sample.2$job.perdis[fill.sample.2$year==t & fill.sample.2$birthselect==1] <- 0
    
    fill.sample.2$birthselect <- NA
...
plot(16:38,babies.nc,type='l',lwd=2,xlab='Age',ylab='Number of children ever born',
     main='Post-Birth Employment Scenario',bty='l')
...
plot(16:38,c(NA,tfr.ncint[-1]),type='l',lty=1,lwd=2,xlab='Age',ylab='Ratio of number of children ever born',
     main='Post-Birth Employment Scenario',ylim=c(0.70,1.30),bty='l')
